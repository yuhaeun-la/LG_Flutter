...
class FirebaseManager { // 기본 크루드 구현
  ...
  // 하위 컬렉션에 데이터 저장
  Future<void> writeSubCollectionData(
      String collection, String docId,
      String subCollection, Map<String, dynamic> data
      ) async {
    try {
  ...
  Stream<QuerySnapshot> listenToSubCollection(
    ...
    try {
      return
        _firestore.collection(collection).doc(docId).collection(subCollection).snapshots();
    } catch (e) {
